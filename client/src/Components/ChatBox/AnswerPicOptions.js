            ...
            <div className="pic-options-wrapper">
                { !this.state.sendLoading ?
                    ...
                    {this.props.data.map( (consultant, index) =>
                        <div className="consultant-card" key={index} onClick={() => {this.setState({sendLoading: true});this.props.history.push({pathname:'/chat/' + consultant.name, state: { name: consultant.name, email:consultant.email }}); this.props.chooseConsultant(consultant.name)}}>
                            {consultant.imgPath ?
